...
class BrokenType:
    ...
    def fix_type_name(self, all_structs : dict):
        ...
        self.type, self.subtypes = BrokenType.get_fixed_type_name_and_subtypes(old_type, all_structs)
    ...
    @staticmethod
    def get_fixed_type_name_and_subtypes(typename, all_structs: dict) -> (str, list[str]):
        inner_match = re_inner_type.search(typename)
        if inner_match is not None:
            replace_with, subtypes = BrokenType.get_fixed_type_name_and_subtypes(inner_match.group(2), all_structs)
            replace_outer_with = BrokenType._get_fixed_single_type_name(inner_match.group(1), all_structs)
        ...
        new_types = [BrokenType._get_fixed_single_type_name(t.strip(), all_structs) for t in typename.split(",")]
    ...
    @staticmethod
    def _get_fixed_single_type_name(typename: str, all_structs: dict) -> str or None:
        if BrokenType._check_invalid_typenames(typename):
            return None

        T = typename.split(" ")
        if len(T) == 3 and T[0] == "enum" and T[1] == "class":  # enum
            return BrokenType._get_fixed_enum_type_name(T[2])
...
class CodeBlock(DumpSelfVars):
    ...
    def __init__(self, re_block_match: re.Match, excluded: bool, source=""):
        ...
        self.body_members.sort(key=lambda bm: bm.name)
        self.body_members.sort(key=lambda bm: bm.type)
    ...
    def generate_class_and_includes(self, all_classes: dict[str, CodeBlock]) -> (str, list[str]):
        ...
        s.append(f"class FSD_API {self.identifier.name} : public {self.parent.identifier.name} {{")
            ...
            for type in bm.subtypes:
                if type in all_classes:
                    if all_classes[type].excluded:
                        ...
                        includes.append(f"{all_classes[type].class_name()}.h")
                    #s += bm.subtypes
            if len(ue_natives) > 0:
        ...
        if self.parent is not None and not self.parent.excluded:
            includes.append(f"{self.parent.class_name()}.h")
        return "\n".join(s), includes
